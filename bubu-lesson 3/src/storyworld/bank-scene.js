/* =====================================================================
   ...
   Scenario engine + Capital One Nessie sandbox client.
...
/* -------------------------------------------------------------------
   1. Nessie client — the same contract as src/services/nessie.js in the
   lesson bundle, compacted for a single-file scene. Live when a key and
   a proxy are configured; otherwise an in-memory bank with identical
   shapes, so the scene never dies mid-demo.
   ------------------------------------------------------------------- */

const NESSIE = {
  // Set these when you drop the scene into the app. '/nessie' is the Vite
  // dev proxy; in production point it at your serverless proxy. Nessie is
  // plain http and its https URL redirects back to http, so a browser on
  // an https page blocks direct calls as mixed content — the proxy is not
  // optional.
  baseUrl: window.__NESSIE_BASE__ || '/nessie',
  apiKey: window.__NESSIE_KEY__ || '',
};

const bankState = { mode: 'mock', reason: '', calls: [] };
...
async function nessieFetch(method, path, body) {
  const join = path.includes('?') ? '&' : '?';
  const url = `${NESSIE.baseUrl}${path}${join}key=${encodeURIComponent(NESSIE.apiKey)}`;
  ...
  try {
    ...
    let json = null;
    let parsed = true;
    try { json = text ? JSON.parse(text) : null; } catch { json = text; parsed = false; }
    if (!res.ok) throw new Error((json && json.message) || `Nessie returned ${res.status}`);
    // A 200 is not proof you reached the API. A single-page-app fallback route,
    // a captive portal, or a proxy misconfigured to serve index.html all answer
    // 200 with HTML — and a client that trusts the status code then runs in a
    // "live" mode where nothing it stores is real. Nessie always returns JSON.
    if (!parsed) throw new Error('Expected JSON from Nessie and got something else — check what the proxy path is actually serving');
    return json && typeof json === 'object' && 'objectCreated' in json ? json.objectCreated : json;
  } finally {
    clearTimeout(timer);
  }
}
...
/** Every bank verb the scene needs, live-or-mock behind one shape. */
const bank = {
  async probe() {
    // Always try the live call rather than gating on a key in the page. The
    // better deployment has the proxy hold the key and the browser hold none,
    // and a page that refuses to try would never discover that it works.
    try {
      const probe = await nessieFetch('GET', '/customers');
      // /customers returns a list. Anything else means we are talking to
      // something that is not Nessie, however cheerful its status code.
      if (!Array.isArray(probe)) throw new Error('That endpoint did not answer like Nessie');
      bankState.mode = 'live';
      bankState.reason = NESSIE.apiKey
        ? 'Connected to the Capital One sandbox. Everything you do here is a real API call.'
        : 'Connected through the proxy, which holds the key. Everything you do here is a real API call.';
    } catch (e) {
      bankState.mode = 'mock';
      bankState.reason = NESSIE.apiKey || NESSIE.baseUrl !== '/nessie'
        ? 'The Capital One sandbox did not answer, so Bubu is banking at the practice branch instead. Nothing in the scene breaks.'
        : 'No sandbox key or proxy is configured, so Bubu is banking at the practice branch. Every call below is the real request shape.';
    }
  },

  async createCustomer(c) {
    logCall('POST', '/customers', `${c.first_name} ${c.last_name}`);
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', '/customers', c); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createCustomer(c);
  },

  async createAccount(customerId, a) {
    logCall('POST', `/customers/${short(customerId)}/accounts`, `${a.type} · ${usd(a.balance)}`);
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', `/customers/${customerId}/accounts`, a); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createAccount(customerId, a);
  },

  async accountsFor(customerId) {
    if (bankState.mode === 'live') {
      try { return await nessieFetch('GET', `/customers/${customerId}/accounts`); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.accountsFor(customerId);
  },

  async createPurchase(accountId, p) {
    logCall('POST', `/accounts/${short(accountId)}/purchases`, `${usd(p.amount)} ${p.description}`);
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', `/accounts/${accountId}/purchases`, p); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createPurchase(accountId, p);
  },

  async createWithdrawal(accountId, w) {
    logCall('POST', `/accounts/${short(accountId)}/withdrawals`, `${usd(w.amount)} ${w.description || ''}`.trim());
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', `/accounts/${accountId}/withdrawals`, w); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createWithdrawal(accountId, w);
  },

  async createDeposit(accountId, d) {
    const payload = { status: 'completed', transaction_date: today(), ...d };
    logCall('POST', `/accounts/${short(accountId)}/deposits`, `${usd(payload.amount)} ${payload.description || ''}`.trim());
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', `/accounts/${accountId}/deposits`, payload); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createDeposit(accountId, payload);
  },

  async createTransfer(fromId, t) {
    logCall('POST', `/accounts/${short(fromId)}/transfers`, `${usd(t.amount)} → ${short(t.payee_id)}`);
    if (bankState.mode === 'live') {
      try { return await nessieFetch('POST', `/accounts/${fromId}/transfers`, t); } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.createTransfer(fromId, t);
  },

  async merchants() {
    if (bankState.mode === 'live') {
      try {
        const list = await nessieFetch('GET', '/merchants');
        if (Array.isArray(list) && list.length) return list;
      } catch { bankState.mode = 'mock'; renderStatus(); }
    }
    return mockBank.merchants();
  },
};
...
const KB = [
  {
    ...
    <p>Savings is where money <strong>sits</strong>. It earns a bit more, and most savings accounts cap you at around six transfers a month — which sounds like a restriction but is really the point. It makes the money slightly annoying to reach, and slightly annoying is what keeps it there.</p>`,
  },
  {
  ...
  },
  {
    ...
    a: `The number people will quote you is three to six months of expenses, and for a college student that can sound absurd. So start smaller: one month of the things that would actually hurt to miss. Rent, food, transit.
  ...
  },
  {
    ...
    <p>And yes, online-only banks pay more — around 4% against a national average nearer 0.6%. Same federal insurance, same access. On $2,000 that gap is about $73 a year. I am not supposed to love telling you that, but you asked.</p>`,
  },
  {
  ...
  },
  {
    ...
    a: `Deposits here are federally insured up to $250,000 per depositor, per bank. If we went under tomorrow, you would be made whole — this is the one part of banking where you genuinely do not need to worry.
  ...
  /* --- credit --- */
  {
    ...
    <p>You get one by borrowing something small and paying it back on schedule. Five things make it up — paying on time is 35%, how much of your limit you are using is 30%, how long your accounts have been open is 15%, and new accounts and credit mix are 10% each. Check yours free at annualcreditreport.com; errors are common.</p>`,
  },
  {
  ...
  },
  {
    ...
    <p>Take the limit and do not spend it. That is the whole trick. If you do not trust yourself with the card in your wallet, leave the card at home — the limit is not what makes you spend.</p>`,
  },
  {
  ...
  /* =============================================== 1. the accounts === */
  {
    ...
    beats: [
      { t: 'say', who: 'tram', text: 'Hi Bubu! What can I help you with today?' },
      {
        ...
        options: [
          ...
          },
          {
            ...
            reply: `Then you came at exactly the right time — a refund landing in an account you have not thought about is how it disappears. We will set up two accounts, checking for what moves and savings for what stays, so the refund has somewhere to go that is not your wallet. Ask me anything first, though.`,
          },
          {
      ...
      },
      {
        ...
        subtitle: 'This is your own money. Whatever you deposit leaves your wallet and lands in the bank.',
        ...
        review(plan, c) {
          ...
          if (checking === 0) {
            return {
              reply: `I will open them, and I want to flag the checking account at zero. Every purchase now becomes a transfer out of savings, and savings caps you at about six transfers a month before the fees start. Move some across when you get a chance.`,
          ...
          if (checking < 100) {
            return {
              reply: `Your instinct is right and the split is a little aggressive. ${usd(checking)} in checking is thin once rent and a phone bill land, and you get about six free transfers a month out of savings. It works — just keep an eye on it.`,
              credits: 20, teaches: 'Checking vs savings',
            };
          }
          const note = kept > total
            ? ` You kept ${usd(kept)} back in your wallet, which is more than you deposited — that is fine, just remember cash in a wallet earns nothing and is the easiest money in the world to spend.`
            : kept > 0
              ? ` And ${usd(kept)} stays in your wallet for the week.`
      ...
      },
      {
        ...
        subtitle: 'This one is a real opt-in. Banks are required to ask.',
        options: [
          {
      ...
      },
      {
        ...
        say: 'Alright. Opening them now — watch the screen, this is the actual record being created.',
        async run(ctx) {
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            set: { limit: 3000 },
            reply: `I can put you in for it, and on paper it is good for your utilization. I will say the quiet part: a $3,000 limit in a first-year student's wallet is a lot of rope. If you take it, the card lives in a drawer and comes out for the bills you already planned.`,
          },
        ],
      },
      {
...
function say(who, html, opts = {}) {
  ...
  if (who === 'sys') {
    ...
    const img = who === 'bubu' ? './bubu.jpg' : './tram.jpg';
...
function typing() {
  const wrap = document.createElement('div');
  wrap.className = 'msg';
  wrap.innerHTML = `<img src="./tram.jpg" alt=""><div class="msg__bubble"><span class="typing"><span></span><span></span><span></span></span></div>`;
  logEl.append(wrap);
  scrollLog();
  return wrap;
}
...
function learn(name) {
  ...
  chip.innerHTML = `<div style="text-align:center"><span class="learned"><span class="tick" aria-hidden="true">✓</span>Learned: ${esc(name)}</span></div>`;
  logEl.append(chip);
  scrollLog();
}

/**
 * The wallet is real money in this game: 1 coin = $1. Learning something pays
 * in, and every deposit you make at the bank pays out. That is the whole point
 * of the scene — you cannot open an account with money you do not have.
 */
function walletDelta(n) {
  if (!n) return;
  ctx.wallet = Math.max(0, Math.round((ctx.wallet + n) * 100) / 100);
  const pill = el('credits');
  el('credits-n').textContent = Math.round(ctx.wallet);
  pill.dataset.bump = 'true';
  setTimeout(() => (pill.dataset.bump = 'false'), 520);
}

const award = (n) => walletDelta(n);
const walletSpend = (n) => walletDelta(-Math.abs(n));
...
function renderStatus() {
  ...
  el('bank-status').textContent = bankState.reason;
}
...
async function renderDash() {
  ...
  if (!accounts.length) {
    note.textContent = 'Nothing opened yet. Your coins are still in your wallet, where they earn nothing.';
    stats.innerHTML = `<div class="stat"><dt>Wallet</dt><dd>${usd(ctx.wallet)}</dd></div>`;
    list.innerHTML = '';
    renderWire();
    return;
  }
  ...
  note.textContent = 'Read back from the bank just now — these are the balances the API holds, not numbers the game is keeping.';
  const limit = ctx.flags.limit || 1500;
  const util = limit ? owed / limit : 0;
  stats.innerHTML = `
    <div class="stat"><dt>Wallet</dt><dd>${usd(ctx.wallet)}</dd></div>
    ...
    <div class="stat"><dt>Net</dt><dd>${usd(ctx.wallet + assets - owed, true)}</dd></div>
...
const TRAM_BRIEF = `You are Tram, a new-accounts associate at a community bank in Greenville, South Carolina. A college student called Bubu is sitting at your desk opening their first accounts.

Answer the student's question in 2-4 short sentences, in plain language, the way a good bank employee actually talks: direct, warm, no jargon you do not immediately explain, no sales pitch. You may use <strong> and <p> tags; no other HTML, no markdown, no headings, no lists.

Rules you do not break:
- Explain how things work. Never tell the student what to do with their own money, never recommend a specific bank, product, stock or investment, and never predict returns.
- If the question needs their personal circumstances, say so and tell them what to weigh instead of deciding for them.
- If the question is not about money, banking, credit, loans or budgeting, say warmly that it is outside what you can help with at this desk and steer back.
- Use current US figures where they matter: federal undergraduate student loans are 6.52% fixed for 2026-27, the average credit card APR on balances carrying interest is about 21.5%, a national average savings account pays about 0.64% while online banks pay around 4.2%, deposits are FDIC insured to $250,000, a typical overdraft fee is up to $35, and credit scores run 300-850.
- Never claim to look anything up, never invent this student's balances, and never mention being an AI.
...
async function askFreeform(text, beat) {
  ...
  if (topic) {
    ...
    award(15);
    ctx.busy = false;
    return renderAsk(beat);
  }

  if (!sampleFn) {
    await tramSays(
      `That one is outside what I can answer off the top of my head, and I would rather not guess at this desk. Try one of the questions on the list — or ask Dr. Bos, that is what he is for.`
    );
    ctx.busy = false;
    return renderAsk(beat);
  }
  ...
  try {
    let started = false;
    const { text: answer } = await sampleFn(`${TRAM_BRIEF}\n\n${text}`, {
      modelTier: 'quick',
      cache: false,
      onText: ({ text: soFar }) => {
        if (!started) {
          started = true;
          bubble.innerHTML = `<img src="./tram.jpg" alt="Tram"><div class="msg__bubble"></div>`;
        }
        bubble.querySelector('.msg__bubble').innerHTML = soFar;
        scrollLog();
      },
    });
    if (!started) {
      bubble.innerHTML = `<img src="./tram.jpg" alt="Tram"><div class="msg__bubble">${answer}</div>`;
    }
    bubble.classList.add('fade-in');
    // A free-form question counts toward the gate, so curiosity is rewarded.
    ctx.asked.add(`free:${Date.now()}`);
    beat.pool.push(`free:${Date.now()}`);
    award(15);
  } catch (e) {
    bubble.remove();
    const copy =
      e && e.code === 'not_granted'
        ? `I cannot pull up an answer to that one right now. Try one of the questions on the list — those I know by heart.`
        : e && e.code === 'rate_limited'
          ? `Give me a moment, I have three people at this desk. Try one of the questions on the list in the meantime.`
          : `That one is outside what I can answer off the top of my head, and I would rather not guess. Try one of the questions on the list, or ask Dr. Bos.`;
    await tramSays(copy, 300);
  }
  ctx.busy = false;
  scrollLog();
  renderAsk(beat);
...
async function runApi(beat) {
  ...
  holder.innerHTML = `<div class="receipt">
      ...
      <pre>${receipt.lines.map(([m, p, d]) => `<b>${m}</b> ${esc(p)}${d ? `\n     → ${esc(d)}` : ''}`).join('\n')}</pre>
...
function renderDebrief(beat) {
  ...
  card.innerHTML = `<div class="receipt">
    ...
    <span><strong>+${beat.credits} credits</strong></span>
  </div>`;
  logEl.append(card);
  scrollLog();
...
/* ----- allocator ---------------------------------------------------------
   ...
   a budget that is genuinely your wallet, and sliders that cannot between them
...
function buildAllocator(host, cfg, onConfirm) {
  ...
  function sync() {
    ...
    avail.innerHTML =
      ...
      `<span>${kept > 0 ? `${usd(kept)} stays in your wallet` : 'Nothing held back'}</span>`;
...
function mountAllocator(beat) {
  const body = clearDock('Set the amounts', `${usd(ctx.wallet)} in your wallet`);
  buildAllocator(
    body,
    { destinations: beat.destinations, available: ctx.wallet, confirm: beat.confirm },
    async (plan, total) => {
      if (ctx.busy) return;
      ctx.busy = true;
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && !el('notifs').hidden) { openNotifs(false); el('bell').focus(); }
});

/**
 * Payday. The money arrives in the wallet first — that is the honest order, and
...
async function armPaycheck() {
  walletDelta(PAYCHECK);
  pushNotif({
    kind: 'paycheck',
    icon: 'coin',
    title: `Payday — ${usd(PAYCHECK)} just landed`,
    body: `Your campus job paid ${usd(PAYCHECK)} and it is sitting in your wallet doing nothing. Coins in a wallet earn no interest and are the easiest money in the world to spend. Where should it go?`,
    action: {
      ...
      destinations: [
        ...
        },
        {
          ...
          color: 'var(--teal)', soft: 'var(--teal-soft)',
          sub: 'The cushion. Around 4% at an online bank.', default: 100,
        },
        {
      ...
      async onConfirm(plan, total, item) {
        item.read = true;
        item.when = 'done';
        item.body = `You put ${usd(plan.checking || 0)} in checking, ${usd(plan.savings || 0)} in savings and ${usd(plan.investing || 0)} into investing. ${usd(PAYCHECK - total)} stayed in your wallet.`;
        renderNotifs();
        openNotifs(false);
        ...
        say('sys', `Payday: ${esc(usd(total))} deposited from your wallet — ${esc(usd(plan.checking || 0))} checking, ${esc(usd(plan.savings || 0))} savings, ${esc(usd(plan.investing || 0))} investing.`);
...
function showTab(which) {
  ...
  const dash = el('dash');
  if (which === 'restart') return location.reload();
...
for (const b of document.querySelectorAll('.nav button')) {
  b.addEventListener('click', () => {
    for (const other of document.querySelectorAll('.nav button')) other.setAttribute('aria-current', 'false');
    b.setAttribute('aria-current', 'true');
    if (b.dataset.nav !== 'storyworld') {
      say('sys', 'That part of Bubu is not wired up in this scene — Storyworld is the one you are in.');
      setTimeout(() => {
        for (const other of document.querySelectorAll('.nav button')) other.setAttribute('aria-current', String(other.dataset.nav === 'storyworld'));
      }, 900);
    }
  });
}
...
(async function boot() {
  ...
  pushNotif(
    {
      kind: 'note',
      icon: 'spark',
      title: 'How your wallet works here',
      body: `You have <strong>${usd(ctx.wallet)}</strong> in coins. Anything you deposit at the bank comes out of it, and anything you learn pays back into it. Events that happen to Bubu will turn up here.`,
      when: 'now',
    },
    { ring: false }
  );
  ...
  // Claude answers anything the question list does not cover. Absent (or
  // declined) simply means free-text falls back to keyword matching, so the
  // scene works either way.
  try {
